...
import { TITLE_STUDENT_LIST_ASSIGNED_TO_SUPERVISOR, TITLE_APPLICATION_LIST_OF_STUDENT } from "../../../Utils/TITLE"
...
function StudentListAssignedSupervisor() {
  ...
  useEffect(() => {
      axios
        .get(
            GET_ALL_STUDENTS_OF_SUPERVISOR + user.id + "/" + currentSession
        )
        .then((response) => {
        ...
        .catch((err) => {
          setErrorMessage("Erreur! Aucun étudiant n'a été assigné à cette session");
          setStudents([]);
        });
  ...
  function showStudent(student) {
    let state = {
      title: TITLE_APPLICATION_LIST_OF_STUDENT + student.firstName  + " " + student.lastName,
