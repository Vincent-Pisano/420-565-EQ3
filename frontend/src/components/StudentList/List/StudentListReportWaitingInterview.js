...
import {
  ...
  ERROR_NO_STUDENT_WAITING_INTERVIEW,
} from "../../../Utils/ERRORS";
import {
  GET_ALL_SESSIONS_OF_STUDENTS,
  GET_ALL_STUDENTS_WAITING_INTERVIEW,
} from "../../../Utils/API";
...
function StudentListReportWaitingInterview() {
  ...
  useEffect(() => {
    if (sessions.length === 0 && currentSession === undefined) {
      ...
      axios
        .get(GET_ALL_STUDENTS_WAITING_INTERVIEW + currentSession)
        .then((response) => {
          setStudents(response.data);
          setErrorMessage("");
        })
        .catch((err) => {
          setErrorMessage(ERROR_NO_STUDENT_WAITING_INTERVIEW);
          setStudents([]);
        });
    }
...
export default StudentListReportWaitingInterview;
