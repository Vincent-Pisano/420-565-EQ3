...
function StudentListReportWithoutSupervisorEvaluation() {
  ...
  useEffect(() => {
    if (sessions.length === 0 && currentSession === undefined) {
      ...
      axios
        .get(GET_ALL_STUDENTS_WITHOUT_SUPERVISOR_EVALUATION + currentSession)
        .then((response) => {
          setStudents(response.data);
          setErrorMessage("");
        })
        .catch((err) => {
          setErrorMessage(ERROR_NO_STUDENTS_FOR_SUPERVISOR_EVALUATION);
          setStudents([]);
        });
    }
...
export default StudentListReportWithoutSupervisorEvaluation;
