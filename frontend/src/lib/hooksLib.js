...
export function useFormFields(initialState) {
  ...
  const workDaysArray = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Sunday', 'Saturday']

  function handleWorkDay(event) {
    let newWorkDays = fields.workDays;
      if (newWorkDays.some(workday => event.currentTarget.id === workday))
        newWorkDays = newWorkDays.filter(workday => event.currentTarget.id !== workday) 
      else
        newWorkDays.push(event.currentTarget.id)
      return newWorkDays
  }
